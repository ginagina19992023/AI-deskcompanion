...
import { callVisionModel as callVisionModelShared } from './vision-provider.js';

const DEFAULT_PROMPT =
  'Describe what is on this computer screen in one short sentence.';

// screen-tip's default prompt differs from vision-provider's generic
// default, so cfg.prompt is filled in here before delegating. The actual
// Ollama/openai-compatible calling code lives in vision-provider.js,
// shared with camera-sense.js.
async function callVisionModel(cfg, base64) {
  return callVisionModelShared({ ...cfg, prompt: cfg.prompt ?? DEFAULT_PROMPT }, base64);
}
